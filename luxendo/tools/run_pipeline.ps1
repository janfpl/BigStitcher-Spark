...
function Initialize-OffsetN5DatasetTool() {
    ...
    if (-not (Test-Path -LiteralPath $javac)) {
        throw "No Java compiler at $javac (this Fiji appears to bundle a JRE only). The background-subtract step (FusionSubtract, on by default) compiles a small Java helper and needs a JDK. Install Zulu JDK 8 + FX and point the tools at it, or run with -FusionSubtract 0."
    }
